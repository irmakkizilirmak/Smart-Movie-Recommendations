...
# 1. ADIM: Arama sonuçlarını çeken bağımsız blok
target_movie = None
if search_query:
    api_key = st.secrets["TMDB_API_KEY"]
...
# 2. ADIM: Ekranda HER ZAMAN görünecek olan Sinematik Boyut Seçim Alanı
...
# 3. ADIM: Ekranda HER ZAMAN görünecek olan Gurme Butonu
if st.button("Discover Gourmet Matches"):
    if target_movie is None:
        st.error("Please search and select a movie from the dropdown first!")
    else:
        ...
        if not df_pool.empty:
            ...
            # Canlı veri setinde cast ve crew olmadığı için çökmemesi adına güvenli taslaklar
