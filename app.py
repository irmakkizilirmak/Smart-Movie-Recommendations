...
# Streamlit session_state (hafıza) yönetimi ile seçilen filmi koruma altına alıyoruz
if 'selected_target_movie' not in st.session_state:
    st.session_state['selected_target_movie'] = None
...
if search_query:
    ...
    if search_results:
        # Arama sonuçlarını listeleyip selectbox'a aktarıyoruz
        ...
        # Seçilen filmi hafızaya kilitliyoruz ki sayfa yenilense de kaybolmasın
        st.session_state['selected_target_movie'] = movie_options[selected_display]
    else:
        st.session_state['selected_target_movie'] = None
        st.warning("No movie found with that name, please try again.")

# 6'lı Sinematik Boyut Seçimi - Arama yapıldıktan sonra ekranda kalıcı olarak belirir
if st.session_state['selected_target_movie'] is not None:
    target_movie = st.session_state['selected_target_movie']
                ...
                with st.container():
                    ...
                    for i, (_, row) in enumerate(results.iterrows(), 1):
                        ...
                        with col2:
                            ...
                            st.markdown(f"⭐ **IMDb:** `{imdb_score}/10`  |  **Tag:** {genres_list}")
